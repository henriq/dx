...
func TestParseHelmChartOverrides_RejectsInvalidServiceName(t *testing.T) {
	chartDirectory := t.TempDir()
	...
	for label, serviceName := range cases {
		t.Run(label, func(t *testing.T) {
			_, err := ParseHelmChartOverrides([]string{serviceName + ":" + chartDirectory})
			require.Error(t, err)
			assert.Contains(t, err.Error(), "invalid service name")
		})
	}
